...
export class User {
  ...
  constructor(private router: Router, public utils: Utils) {
    ...
    FlightService.getDestinations().then(rsp => this.destinations.set(rsp.data));

    FlightService.getFlightToDestination(this.activeUser!.destination).
  ...
  updatePassword() {
    Alerts.confirm('Are you sure you want to change password?', () => {
      if (this.oldPassword != this.activeUser?.password) {
        Alerts.error('invlaid old password');
        return;
      }
      ...
      if (this.newPassword != this.passRepeat) {
        Alerts.error('Password dont match');
        return;
      }
